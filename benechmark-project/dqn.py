...
if scenario == "lust":
    ###worng path
    lust_file_name = "D:/study/Projects/faculty/graduation project/old_project/datasets/dataset-lust-tl-clusters.csv"
    ...
    alex_file_name = "D:/study/Projects/faculty/graduation project/old_project/datasets/dataset-alex-tl-clusters.csv"
    alex_raw_df = pd.read_csv(alex_file_name)
    alex_raw_df['trafficlight_count'] = alex_raw_df['trafficlight_count'].map(lambda x: ast.literal_eval(x)) #converted to list
    df = alex_raw_df
...
agent_infos = []
for (cluster_id, tl_ids, count) in clusters:# i think cluster_id, tl_ids, count not used,just used tl_ids as expected
...
while True:
    for agent_info in agent_infos: # multiagent ??? understand  yes??????????/
        agent = agent_info[0]
